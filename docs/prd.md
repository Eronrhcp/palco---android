...
Este MVP existe para provar que um músico consegue, a partir de um link do Cifra Club, montar rapidamente uma setlist de cifras pronta para leitura em performance — com controle de fonte, tom e velocidade por música — e acessá-la de forma confiável mesmo sem conexão à internet no momento da apresentação, sem necessariamente ter organização avançada de repertório, sincronização em nuvem, ou gestão de shows/local.
...
| Criar setlists, adicionar/remover músicas, ordenar músicas na setlist                | Organização mínima pra "repertório pronto pra leitura" fazer sentido |
| Configuração de fonte, tom, velocidade por combinação setlist+Música                 | Requisito explícito desde a ideia original |
...
| Busca de música (dentro de uma setlist e em todas as setlists)                         | Resolve diretamente a dor da "canja" (Seção 1) |
| Última configuração usada por música (fonte/tom/velocidade), independente da setlist | Preserva a intenção do usuário ao abrir música fora de contexto de setlist |
...
| Configurações gerais avançadas da setlist (ritmo, metadados extras) | Ordenação e nome básico já bastam pra provar a hipótese |
...
| Compartilhamento de setlists entre usuários                         | Feature nova, não validada ainda |
...
| RF-07 | O sistema deve permitir que, no momento da importação, o usuário associe a música a uma setlist existente ou crie uma nova setlist. |

### Repertório (Setlists e Músicas)

| ID | Requisito |
|---|---|
| RF-08 | O sistema deve permitir que o usuário crie uma setlist, definindo um nome. |
| RF-09 | O sistema deve permitir adicionar uma música já existente no repertório a uma setlist. |
| RF-10 | O sistema deve permitir remover uma música de uma setlist, sem excluir a música do repertório geral. |
| RF-11 | O sistema deve permitir reordenar as músicas dentro de uma setlist. |
| RF-12 | O sistema deve permitir renomear ou excluir uma setlist. |

### Configuração por música

| ID | Requisito |
|---|---|
| RF-13 | O sistema deve permitir definir fonte, tom e velocidade para uma música, vinculados à combinação setlist+música específica. |
| RF-14 | O sistema deve persistir localmente as alterações de fonte/tom/velocidade feitas dentro de uma setlist. |
| RF-15 | O sistema deve manter e atualizar, para cada música, a última configuração usada (fonte/tom/velocidade), independente da setlist em que foi tocada. |
...
| RF-28 | O sistema deve permitir avançar para a próxima música da setlist, quando a música atual foi aberta no contexto de uma setlist. |
| RF-29 | O sistema deve permitir retornar para a música anterior da setlist, quando a música atual foi aberta no contexto de uma setlist. |
| RF-30 | O sistema deve ocultar ou desabilitar os controles de próxima/anterior quando a música foi aberta fora do contexto de uma setlist (ex: via busca global). |

### Busca

| ID | Requisito                                                                                                                       |
|---|---------------------------------------------------------------------------------------------------------------------------------|
| RF-21 | O sistema deve permitir buscar uma música por título ou artista dentro de uma setlist específica.                               |
| RF-22 | O sistema deve permitir buscar uma música por título ou artista em todo o repertório (todas as setlists).                       |
| RF-23 | O sistema deve permitir abrir uma música diretamente a partir dos resultados de busca, sem exigir que ela pertença a uma setlist. |
...
| RF-25 | O sistema deve armazenar localmente setlists, ordenação e configurações por música, disponíveis sem conexão.                                                      |
...
| ID | Requisito |
|---|---|
| RNF-01 | Após uma música ter sido importada e salva localmente, 100% das funcionalidades de leitura (teleprompter, transposição, navegação entre músicas da setlist) devem operar sem nenhuma chamada de rede. |
...
| ID | Requisito |
|---|---|
| RNF-04 | A troca entre músicas de uma setlist (RF-28/RF-29) deve ocorrer em até 300ms, por se tratar de dado já persistido localmente — este é o requisito que resolve diretamente o problema da "canja" descrito na Seção 1. |
...
| Repertório | Criar, editar, reordenar e excluir setlists/músicas reflete corretamente após reabrir o app.                                                             |
| Configuração por música | Alterar config numa setlist não afeta a mesma música em outra setlist. Fora de setlist, mostra última configuração usada.                                      |
| Motor de transposição | 100% de aprovação em suíte de testes unitários com casos comuns e de borda.                                                                              |
| Teleprompter | Rolagem, pausa, ajustes em tempo real, reinício e navegação funcionam sem travamento. Próxima/anterior ocultos fora de contexto de setlist.                |
| Busca | Retorna resultado correto dentro de uma setlist e em todo o repertório, dentro do tempo definido em RNF-05.                                                |
...
3. Navegação por próxima/anterior no teleprompter só existe em contexto de setlist; fica oculta quando a música é aberta via busca global — motivo: evitar ambiguidade de "sequência" sem contexto definido.
4. Música aberta fora de uma setlist usa a última configuração (fonte/tom/velocidade) usada, não um padrão neutro — motivo: preserva a intenção mais recente do usuário, menor surpresa.
